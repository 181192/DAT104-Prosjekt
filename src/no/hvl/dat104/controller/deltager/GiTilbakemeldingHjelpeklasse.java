...
public class GiTilbakemeldingHjelpeklasse {
	...
	/**
	 * Sjekker om man må vente basert på parameter og eventuell tidligere avgitt
	 * stemme
	 * 
	 * @param request
	 * @param sekunderVente
	...
	public static void settFeilmelding(HttpServletRequest request) {
		...
		if (sistStemme == null) {
			...
			Date naa = new Date();
			long sekunderIgjen = GiTilbakemeldingHjelpeklasse.sekunderVente
					- ((naa.getTime() - sistStemme.getTime()) / 1000);
			if (sekunderIgjen > 1) {
	...
	public static void settInfomelding(HttpServletRequest request) {
		if (request.getSession().getAttribute("avsluttendeStemmeAvgitt") != null
				&& (boolean) request.getSession().getAttribute("avsluttendeStemmeAvgitt") == true) {
